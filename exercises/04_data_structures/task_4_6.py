...
ospf_route_l = ospf_route.split()
prefix = ospf_route_l[0]
metric = ospf_route_l[1].strip('[]') 
n_h = ospf_route_l[3].strip(',') 
l_u = ospf_route_l[4].strip(',') 
out_i = ospf_route_l[5]
template = (f'''
            Prefix               {prefix}
            AD/Metric            {metric}
            Next-Hop             {n_h}
            Last update          {l_u}
            Outbound Interface   {out_i}
            ''')
print(template)
